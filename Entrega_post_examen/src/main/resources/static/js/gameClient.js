...
window.GameClient = (() => {
    ...
    function handleWSMessage(msg) {
        ...
        switch (data.type) {
            ...
            case "question":
                ...
                (function() {
                    const wcModal = document.getElementById('wildcardModal');
                    if (wcModal) {
                        const wcInst = bootstrap.Modal.getInstance(wcModal);
                        if (wcInst) wcInst.hide();
                    }
                })();
            ...
            // si el jugador cayó en un comodín se muestra el selector de categoría
            case "wildcard":
                if (data.players) renderPlayers(data.players);
                if (data.gameState) currentGameState = data.gameState;
                function mostrarSelectorComodin() {
                    if (!isRolling) {
                        initBoard();
                        actualizarVisibilidadDado();
                        mostrarModalComodin(data.categories || []);
                    } else {
                        setTimeout(mostrarSelectorComodin, 500);
                    }
                }
        ...
        return go(`/game/${cfg.gameCode}/players`, "GET")
            .then(data => {
                ...
                if (isMatch) {
                    ...
                    const comodinPendiente = infoTurno.subState === "WILDCARD_PENDING";
                    ...
                    } else if (preguntaPendiente && preguntaActiva) {
    ...
    function initBoard() {
        ...
        function drawPlayers(cellGroup, pos, cellSize) {
            ...
            const playersHere = players.filter(p => {
                ...
                if (boardPos > boardSize) boardPos = goalPos;
                return boardPos === pos;
            });
        ...
        const boardSize = cfg.boardSize || 64;
        const gridCols = 8;
        const rows = boardSize / gridCols;
        const lastRow = Math.floor((boardSize - 1) / gridCols);
        const goalOnRight = (lastRow % 2 === 0);
        const goalX = goalOnRight ? gridCols + 1 : 0;
        const cols = gridCols + 2;
        const goalPos = boardSize + 1;

        //funcion interna para saber en qué coordenadas dibujar la casilla pos
        function getBoardCoords(pos) {
            if (pos === 0) return { x: 0, y: rows -1 };
            if (pos === goalPos) {
                return { x: goalX, y: 0 };
            }
            const adjPos = pos - 1;
            ...
            return { x: col + 1, y: rows-1 - rowFromBottom };
        }
        ...
        function redraw() {
            ...
            for (let i = 0; i <= goalPos; i++) {
                ...
                if (i === 0 || i === goalPos) {
                    ...
                    const esComodin = (cfg.casillasComodin || []).includes(i);
                    const fill = categoryKey ? categoryColorMap[categoryKey]: '#f2f2f2';
                    const strokeColor = esComodin ? '#FFD700': '#999';
                    const strokeWidth = esComodin ? 3: 1;
                    ...
                    if (esComodin) {
                        group.text('★')
                            .font({size: cell / 4, anchor: 'middle'})
                            .center(cell / 2, cell * 0.72);
                    }
    ...
    function gestionarModalPregunta(activeQuestion) {
        ...
        const donContainer = document.getElementById("double-or-nothing-container");
        const donCheck = document.getElementById("double-or-nothing-check");
        if (donContainer && donCheck) {
            donCheck.checked = false; // siempre empieza sin marcar
            if (!miTurno || q.answered) {
                // Si no es mi turno o ya se respondió, ocultamos el checkbox
                donContainer.classList.add("d-none");
            } else {
                donContainer.classList.remove("d-none");
            }
        }
    ...
    function enviarRespuestaMulti(answer) {
        ...
        const donCheck = document.getElementById("double-or-nothing-check");
        const doubleOrNothing = donCheck ? donCheck.checked : false;
        ...
        // Ocultamos el checkbox una vez enviada la respuesta
        const donContainer = document.getElementById("double-or-nothing-container");
        if (donContainer) donContainer.classList.add("d-none");

        // pasamos la respuesta al backend para que sea validada
        go(`/game/${cfg.gameCode}/submit-answer`, "POST", { answer: answer, doubleOrNothing: String(doubleOrNothing) })
            .then(data => {
    ...
    // Muestra el modal de selección de categoría para casillas comodín
    function mostrarModalComodin(categories) {
        // Solo el jugador activo puede elegir categoría
        const isMyTurn = currentGameState &&
            Number(currentGameState.currentTurnPlayerId) === Number(cfg.currentUserId);

        const modal = document.getElementById('wildcardModal');
        const grid = document.getElementById('wildcard-categories-grid');
        const notice = document.getElementById('wildcard-waiting-notice');
        const title = document.getElementById('wildcard-modal-title');
        ...
        if (isMyTurn) {
            if (title) title.textContent = '★ ¡Casilla Comodín! Elige tu categoría';
            if (notice) notice.classList.add('d-none');
            grid.classList.remove('d-none');

            categories.forEach(cat => {
                const btn = document.createElement('button');
                btn.className = 'btn btn-outline-warning fw-bold wildcard-cat-btn';
                btn.textContent = cat.label;
                btn.onclick = () => chooseWildcardCategory(cat.key);
                grid.appendChild(btn);
            });
        } else {
            if (title) title.textContent = '★ Casilla Comodín';
    ...
    // Envía la categoría elegida al servidor
    function chooseWildcardCategory(categoryKey) {
        const modal = document.getElementById('wildcardModal');
        if (modal) {
        ...
        go(`/game/${cfg.gameCode}/choose-category`, 'POST', { category: categoryKey })
            .then(data => {
                if (data.status !== 'success') {
                    console.error('Error eligiendo categoría comodín:', data.message);
                }
            })
            .catch(err => console.error('Error en chooseWildcardCategory:', err));
    }
