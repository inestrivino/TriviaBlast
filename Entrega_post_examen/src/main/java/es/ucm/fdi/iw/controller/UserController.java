...
public class UserController {
  ...
  public String profile(Model model, HttpSession session) {
    ...
    List<Player> lastGames = entityManager
      .createNamedQuery("Player.lastFiveByUser", Player.class)
      .setParameter("userId", freshUser.getId())
      .setMaxResults(5)
      .getResultList();
  ...
  public String scoreboard(Model model, Authentication authentication) {
    ...
    List<Object[]> weeklyRows = entityManager.createQuery(
        "SELECT p.user, SUM(p.points) FROM Player p " +
        "WHERE p.date >= :weekStart AND p.user.enabled = true " +
        "GROUP BY p.user ORDER BY SUM(p.points) DESC", Object[].class)
